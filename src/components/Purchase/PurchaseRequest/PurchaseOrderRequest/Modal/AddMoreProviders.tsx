import { Backdrop, Box, Button, Chip, CircularProgress, MenuItem, Stack, TextField, Typography } from '@mui/material';
...
export const AddMoreProviders = (props: AddMoreProvidersProps) => {
  ...
  const [providersClone, setProvidersClone] = useState(providersData.flatMap((p) => p.proveedor));
  ...
  const updateProvidersClone = () => {
    ...
    const updatedProvidersClone = providersData
      .flatMap((p) => {
        const provider = providers.find((provider) => provider.id === p.proveedor.id_Proveedor);
        if (provider) {
          return provider.id;
        }
        return null;
  ...
  const handleSubmit = async () => {
    if (providersClone.length < 3) return toast.error('Selecciona al menos 3 proveedores para continuar!');
    setIsFetching(true);
    try {
      await selectManyProvidersForTender(requestPurchaseInfo.purchaseOrderId, providersClone);
    ...
    <Box sx={{ ...style, ...styleBar }}>
      ...
      <Stack spacing={2} sx={{ bgcolor: 'white', p: 4 }}>
        ...
        <TextField
          ...
          onChange={(e) => {
            if (providersClone.length === 3 && providersClone.some((i) => i === providerSelectedId)) {
