...
interface Action {
  ...
  fetchData: (pageSize: number, search: string, enabled: boolean, pageIndex: number) => Promise<void>;
}

export const useUserPaginationStore = createWithEqualityFn<State & Action>((set) => ({
  count: 0,
  pageCount: 0,
  resultByPage: 0,
  pageIndex: 0,
  ...
  userDisabled: false,
  newUser: null,
  setEnabled: (state: boolean) => set(() => ({ enabled: state, pageIndex: 0, isChecked: [] })),
